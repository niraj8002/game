...
const RechargeAdmin = () => {
  ...
  const getPayment = () => {
    axiosInstance.get(`/payment`).then((response) => {
  ...
  const removePayment = (obj) => {
    ...
    Modal.confirm({
      ...
      cancelText: "No",
      onOk() {
        axiosInstance.put(`/payment/${obj._id}` ,{status:"failed"}).then((res) => {
  ...
  const confirmPayment = (obj) => {
    ...
    Modal.confirm({
      ...
      onOk() {
       ...
       if (obj.status==="requested"){ axiosInstance.put(`/payment/${obj._id}` ,{status:"success"}).then((res) => {
          if (res.data && res.data.responseCode === 1) {
             const data = {
